...
def canonicalize_smiles(smiles):
    ...
    try:
        return Chem.MolToSmiles(Chem.RemoveHs(Chem.MolFromSmiles(Chem.MolToSmiles(mol))))
    except:
        return None
...
class ProductStock(ContextCollection):

    _collection_name = "product_stock"
    ...
    def __init__(self, product: str) -> None:
        ...
        self._stop_criteria: StrDict = {"amount": None, "price": None, "counts": {}}
        self._use_stop_criteria: bool = False
        self.product = canonicalize_smiles(product)

    def __contains__(self, mol: Molecule) -> bool:
        mol_smiles = [canonicalize_smiles(sml) for sml in mol.smiles.split(".")]
        # print(mol_smiles, self.product)
        if self.product not in mol_smiles:
            return False
        else:
            return True
